        ...
        <nav class="navbar navbar-expand-lg">
            ...
            <div class="collapse navbar-collapse" id="navbarNavDropdown">
                <ul class="navbar-nav feature-box ms-auto">
                    ...
                    @endguest
                    @auth
                    <li><a href="{{ route('back.index') }}" class="me-1 btn btn-round btn-kikyo"><i class="fas fa-cogs"></i>
                            後台管理</a></li>
                    <li><a href="{{ route('logout') }}" class="me-1 btn btn-round btn-danger" onclick="if(confirm('您確定要登出嗎?')) javascript:location.href='{{ route('logout') }}';else return false"><i class="fas fa-sign-out-alt"></i>
                            登出系統</a></li>
                    @endauth
                    @impersonating($guard = null)
                    <li><a href="{{ route('sims.impersonate_leave') }}" class="me-1 btn btn-round btn-secondary" onclick="return confirm('結束模擬？')"><i class="fas fa-sign-out-alt"></i>
                            結束模擬</a></li>
                    @endImpersonating
                </ul>
            </div>
